...
; -------------------------------------------------------------------------
	ORG	70
	DB	00	; Data byte - could be a table here.
			; Interrupt code starts here.
...
;    bytes for key press storage. The 16 locations are used in a
;    circular fashion.
;
; 27) Create a new interrupt that puts characters onto the next
;    free screen location. See if you can get correct behaviour
;    in response to the Enter key being pressed (fairly easy) and
;    if the Back Space key is pressed (harder).
;
; -------------------------------------------------------------------------